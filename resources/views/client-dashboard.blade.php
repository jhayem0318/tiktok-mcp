...
<section class="section"><div class="section-head"><div><div class="eyebrow">Monthly history</div><h2>Saved months</h2><p>Select a saved month to view its database snapshot without requesting live data.</p></div></div><div class="tabs">@forelse($monthlyHistory as $month)<a class="tab" href="{{ route('client.dashboard', ['shop_id'=>$selectedShop->shop_id,'start_date'=>$month->period_start->toDateString(),'end_date'=>$month->period_end->toDateString()]) }}">{{ $month->period_start->format('M Y') }} · {{ $month->orders_complete ? 'Saved' : 'Incomplete' }}</a>@empty<p class="empty">No months saved yet. Historical months must first be imported from the API.</p>@endforelse</div></section>
